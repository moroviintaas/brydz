...
use crate::options::contract_generation::{ChoiceDoubling, ForceDeclarer, Subtrump};
use crate::options::contract_generation::GenContractOptions;
use crate::options::DealMethod;

fn generate_single_contract(params: &GenContractOptions, rng_source: &mut impl Rng) -> Result<ContractGameDescription, BrydzModelError>{
    ...
    let contract_value = rng_source.random_range(params.min_contract..=params.max_contract);
    let trump: Trump = match params.trump_limit{
        Subtrump::All => TrumpGen::<Suit>::random(rng_source),
        Subtrump::Colored => {
            Trump::Colored(Suit::random(rng_source))
        },
        Subtrump::NoTrump => Trump::NoTrump
    };
    let contract_declarer: Side = match params.force_declarer {
        ForceDeclarer::DontForce => Side::random(rng_source),
    ...
    let doubling = match params.choice_doubling{
        ChoiceDoubling::Any => *[Doubling::None, Doubling::Redouble, Doubling::Redouble].choose(rng_source).unwrap(),
    ...
    let (template, cards) = match params.deal_method{
        ...
        DealMethod::Biased => {
            let mut rng = rng();
            let distribution: BiasedHandDistribution = rng.random();
